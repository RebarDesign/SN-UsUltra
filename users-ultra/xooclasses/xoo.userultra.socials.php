...

{
...

	{
		...
		if($this->check_if_sent($receiver_id )) {
...

			{
				...
				echo __(" Student Added ", 'xoousers');
				...
				echo __("You have to be logged in to add a student ", 'xoousers');
			...
			echo __("Student already added", 'xoousers');
...

	{
        ...
        if ( in_array( 'tutor', $rd_current_user->roles ) && ($rd_user_role == 'student')) {
			$html.= "<div class='uultra-friend-request-box'>";
			$html.= "<p class='total_likes' id='uu-friends-total-id-".$user_id."'>". $total." ".__('Tutors','xoousers')." </p>";
			$html .= '<a class="uultra-btn-friend" id="uu-send-friend-request" href="#" user-id="'.$user_id.'" title="'.__('Add Student','xoousers').'"><span><i class="fa fa fa-users fa-lg"></i>  Add to my students</span> </a>';
			$html.= "</div>";
	   	};
...

	{
...

		{
			...
			$sql = "DELETE FROM " . $wpdb->prefix . "usersultra_friends  WHERE friend_id  = '$item_id' AND friend_sender_user_id = '$logged_user_id'";



			$message = __('Student Removed','xoousers'); 			}
